...
server <- shinyServer(function(input, output) {
  ...
  if(interactive()){
    ...
    g4.2 <- ggplot(filter(g4, chr == "n"), aes(x=chr, y=category, fill=prop)) +
      geom_tile(color = "white", size = 1) + 
      scale_fill_gradient(low = "white", high = "black" ) +
      theme_classic() + theme(axis.ticks = element_blank(),
                              axis.line = element_blank(),
    ...
    gridExtra::grid.arrange(g4.1, g4.2, ncol=2, widths=c(4,1), heights = c(1,1))
    #library(cowplot)
    plot_grid(g4.1, g4.2, ncol = 2, align = "h", rel_heights = c(1, 1))
  ...
  for (i in 1:max_plots) {
    local({
      ...
      output[[plotname]] <- renderPlot({
        ...
        #plot_list <- list()
        #for(ea_chr_pair in 1:ncol(chr_pairs)){
        print("all_combos_chr_pairs_and_classes()")
        print(all_combos_chr_pairs_and_classes())
