...
create_perc_matr2.1 <- function(matr, title, minChr, maxChr, xlab, ylab){
  ...
  x <- ggplot(matr, aes(x = V1, y = V2, fill = log(prop*100+1, 10))) + 
    ...
    xlab(xlab) + 
    ylab(ylab) + 
    scale_x_continuous(breaks=seq(minChr, maxChr, 1), labels=as.character(c(paste0("<", minChr+1),{minChr+1}:{maxChr-1},paste0(">", maxChr-1)))) + 
    scale_y_continuous(breaks=seq(minChr, maxChr, 1), labels=as.character(c(paste0("<", minChr+1),{minChr+1}:{maxChr-1},paste0(">", maxChr-1)))) + 
