...
server <- shinyServer(function(input, output, session) {
  ...
  fishNm <- list.files(here("testDat/data_in_app/"), pattern = "^Treat|^Contr")
  fishPath <- here("testDat/data_in_app/", list.files(here("testDat/data_in_app/"), pattern = "^Treat|^Contr"))
  
  #observe({
  #  req(input$fish_files)
    f1 <- retFishDf(fish_name = fishNm, fish_datapath = fishPath)
  #})
  ...
  #observe({
    #req(input$sky_file)
    skyPath <- here("testDat/data_in_app/test_sky_data (1).xlsx") #here("testDat/", here("testDat/test_sky_data.xlsx")))
    s1 <- retSkyDf(sky_datapath = skyPath)
  #})
  
  #observeEvent(input$reset_sky, {
   # s1 <- NULL
   wgsCNPath <- here("testDat/data_in_app/sc_wgs_copy_number_dat (1).txt")  #list.files(here("testDat/"), pattern = "test_aneupl_file*")
   wgsKeyPath <- here("testDat/data_in_app/sc_wgs_key_test.xlsx")#, list.files(here("testDat/"), pattern = "test_aneupl_file*"))
    
   w1 <- retWgsDf(wgs_datapath = wgsCNPath, wgs_key_datapath = wgsKeyPath)
 ...
 #})
  ...
  #      )     
        ...
        numX = as.numeric(1)#numbX())
        numY = as.numeric(1)#numbY())
        list_to_pass <- list(s1, f1, w1) %>% purrr::compact() #2018-05-05 issue here?
...
#                    search = list(regex = TRUE, caseInsensitive = FALSE))) %>% DT::formatRound(c(4:11), 2)
 ...
 #       )     
        ...
        sumStatsPerChr <- purrr::reduce(list(aneupl_scores,heterog_scores, anca_scores_normalized), full_join, by=c("category","file_type", "chr")) %>%
          select(category, file_type, n, everything())
  #      return(sumStats)
 #     })
